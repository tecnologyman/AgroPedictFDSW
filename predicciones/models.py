...
class DatosPredioHistorico(models.Model):
    ...
    def __str__(self):
        return f"{self.usuario} - Año {self.ano}: {self.rendimiento_real}"
...
# MODELO PARA ANÁLISIS DE PREDICCIONES
...
class ModeloLSTM(models.Model):
    """Modelo LSTM entrenado para predicciones"""
    nombre = models.CharField(max_length=100)
    ...
    class Meta:
        db_table = 'predicciones_modelolstm'
    
    def __str__(self):
        return f"{self.nombre} v{self.version}"

class PrediccionLSTM(models.Model):
    ESTADO_CHOICES = [
        ('pendiente', 'Pendiente'),
        ('completada', 'Completada'),
        ('error', 'Error'),
    ...
    prediccion_original = models.OneToOneField(
        'Prediccion',
        on_delete=models.CASCADE,
        related_name='prediccion_lstm'
    )
    modelo = models.ForeignKey(ModeloLSTM, on_delete=models.PROTECT)
    
    rendimiento_predicho_ano_actual = models.FloatField(null=True, blank=True, default=0.0)
    ...
    def __str__(self):
        return f"LSTM - {self.prediccion_original.id}"
