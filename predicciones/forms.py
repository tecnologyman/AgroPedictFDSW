...
# =====================================================
# FORMULARIO PRINCIPAL — NUEVA PREDICCIÓN
# =====================================================
class PrediccionForm(forms.ModelForm):
    class Meta:
        model = Prediccion
        fields = [
            'tipo_arbol', 'comuna', 'hectareas',
            'edad_arboles', 'densidad_plantacion',
            'tipo_riego', 'tipo_suelo', 'fertilizacion'
        ]
    ...
    def __init__(self, *args, **kwargs):
        """Carga dinámica de queryset para selects."""
        super().__init__(*args, **kwargs)
        # Evita consultas si la BD está vacía (modo demo o migraciones recientes)
...
# =====================================================
# FORMULARIO — ANÁLISIS DE PREDICCIÓN
# =====================================================
class AnalisisPrediccionForm(forms.Form):
    prediccion = forms.ModelChoiceField(
        queryset=Prediccion.objects.none(),
        widget=forms.Select(attrs={
            'class': 'form-control',
            'required': True,
        }),
        label='Seleccionar Predicción para Analizar',
        help_text='Elija una predicción completada para realizar un análisis detallado.',
    )

    def __init__(self, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)

        # Filtra predicciones completadas del usuario (o vacío si no hay usuario)
        qs = (
            Prediccion.objects.filter(estado='completada', usuario=user)
            .select_related('tipo_arbol', 'comuna')
            .order_by('-fecha_creacion')
            if user
            else Prediccion.objects.none()
        )
        self.fields['prediccion'].queryset = qs

        # Etiquetas legibles para el combo
        choices = [('', 'Seleccione una predicción')]
        for p in qs:
            label = f"{p.tipo_arbol} - {p.comuna} - {p.fecha_creacion.strftime('%d/%m/%Y')}"
            if p.roi_proyectado:
                label += f" (ROI: {p.roi_proyectado:.1f}%)"
            choices.append((p.id, label))
        self.fields['prediccion'].choices = choices


# =====================================================
# FORMULARIO — COMPARACIÓN DE PREDICCIONES
# =====================================================
class ComparacionPrediccionesForm(forms.Form):
    predicciones = forms.ModelMultipleChoiceField(
        ...
        help_text='Seleccione entre 2 y 5 predicciones completadas para comparar.',
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        qs = (
            Prediccion.objects.filter(estado='completada')
            .select_related('tipo_arbol', 'comuna')
            .order_by('-fecha_creacion')
        )
        self.fields['predicciones'].queryset = qs

    def clean_predicciones(self):
        """Valida cantidad de predicciones seleccionadas."""
